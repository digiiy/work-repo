...
-- 데이터를 조건을 붙여 조회하기
-- 예제 1) 월급(SALARY)이 1500 이상인 사원(EMPLOYEE) 조회하기(ENAMEM SALARY)
...
-- 연습 1) 급여가(SALARY) 1000과 1500사이의 사원 조회하기(전체 조회)
-- 사원 : EMPLOYEE
...
-- 사용법) SELECT 컬럼명 FROM 테이블명 WHERE NOT 컬러명 = 값; (그 값에 반대되는 데이터가 출력됨)
SELECT * FROM EMPLOYEE
WHERE NOT DNO = 10;
...
-- 연습 3) 커미션(COMMISSION)이 300이거나 500이거나 1400인 사원 출력하기
-- 사원 : EMPLOYEE
SELECT * FROM EMPLOYEE 
...
-- NOT BETWEEN
-- 예제 9) 급여가 1000미만이거나 1500 초과인 사원 조회하기
-- 사원 : EMPLOYEE
SELECT * FROM EMPLOYEE
WHERE SALARY < 1000
OR    SALARY > 1500;
...
-- 연습 3) 1982년에 입사한 사원(HIREDATE) 조회하기
...
-- 사원 : EMPLOYEE
SELECT * FROM EMPLOYEE
WHERE CCOMMISSION = 300
OR    CCOMMISSION = 500
OR    CCOMMISSION = 1400;

-- 사용법) SELECT 컬럼명 FROM 테이블명 WHERE 컬럼명 IN (값, 값2, ...);
-- 사용처 ; OR로 연결된 데이터를 간단하게 사용 가능
SELECT * FROM EMPLOYEE
WHERE COMMISSION IN (300, 500, 1400);

-- NOT IN 사용
-- 예제 11) 상여금(COMMISSION) 300이 아니고, 500이 아니고, 1400이 아닌 사원 조회
-- 사원 : EMPLOYEE
SELECT * FROM EMPLOYEE
...
-- 예제 14) IN, BETWEEN의 반대를 나타낼때 NOT 앞에 붙여서 사용했음
-- LIKE의 반대를 나타낼때 똑같이 사용할 수 있음 : NOT LIKE
...
-- IN, BETWEEB, LIKE의 반대는 앞에 NOT
-- IS NOT NULL 있음 (NULL 값을 가지지 않은 사람)
-- 상여금에(COMMISSION) NULL이 아닌 사람을 조회하세요
SELECT * FROM EMPLOYEE
...
-- 연습 7) 사원 테이블(EMPLOYEE)에서 사원명(ENAME)으로 오름차순 정렬해서 조회하세요
-- 사원 : EMPLOYEE
SELECT * FROM EMPLOYEE
ORDER BY ENAME ASC; --ASC(오름차순, 생략가능)
...
-- 연습 9) 사원 테이블(EMPLOYEE)에서 급여는(SALARY) 내림차순으로 정렬하고,
