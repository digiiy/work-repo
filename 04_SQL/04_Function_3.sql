...
-- 예제 2) 급여을 출력하면서 통화기호를 (W, $등) 포함해서 출력하세요
-- 숫자 포맷 : L - 각 나라별 통화 기호를 자동으로 붙여줌(W, $등)
--            9 - 3자리를 잡고 자리수가 모자라도 따로 0을 채워주지 않음
--            0 - 3자리를 잡고 자리수가 모자라면 0을 채움
--            , - 각 통화의 중간에 쉼표(,) 자동으로 추가해줌
SELECT ENAME, SALARY
      ,TO_CHAR(SALARY, 'L999,999')
      ,TO_CHAR(SALARY, 'L000,000')
FROM EMPLOYEE;

-- 3) TO_DATE(문자[숫자], 날짜 포맷) : 문자[숫자] -> 날짜형으로 바꾸어 주는 함수
SELECT ENAME, HIREDATE
FROM EMPLOYEE
-- WHERE HIREDATE = '19810220'; -- 자동 자료형 변환(내부적으로 문자 -> 날짜형으로 바뀜)
WHERE HIREDATE = TO_DATE('19810220', 'YYYYMMDD');

-- 4) TO_NUMBER(문자, 숫자 포맷) : 문지 -> 숫자로 변환하는 함수
-- 예제 4) 숫자 형태의 문자 빼기
...
-- 사용법) ,CASE WHEN DNO= 조건식1 THEN ' 결과1'
--              WHEN DNO= 조건식2 THEN ' 결과2'
--              ...            
--              ELSE '기본결과'
--          END
SELECT ENAME, DNO
