...
--       1-2) 그 급여를 비교해서 많이 받는 사원을추가로 찾아야함
...
-- 2) 다중행 서브쿼리(자식쿼리) : 여러 건이 나옴
-- 예제 3) 1. 부서별 최소 급여을 받는 사원중에서
--         2. 사원번호와 이름을 출력하세요.
-- 1. 부서번호별 최소 급여
SELECT MIN(SALARY)
FROM EMPLOYEE
...
-- 2. (950, 1300, 800) 급여를 받는 사원번호(ENO), 이름(ENAME)을 출력
SELECT ENO, ENAME
FROM EMPLOYEE
...
SELECT ENO, ENAME
FROM EMPLOYEE
WHERE SALARY IN (SELECT MIN(SALARY)  -- 여러건에선 IN을 쓴다 ex) (950, 1300, 800)
FROM EMPLOYEE
GROUP BY DNO);    
