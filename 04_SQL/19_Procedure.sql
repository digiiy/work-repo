-- 19_Procedure.sql
-- pl/sql : 프로시저/평선, DB 프로그래밍 분야(SQL 숙련자들)
-- 난이도 높음
-- 소스 : Oracle DB에 저장됨
-- 장정 : 1) 성능향상
-- 자바/js 처럼 코딩하는 것 말함
...
-- 1) 선언부
DECLARE
-- 변수 초기화, 변수 정의 : 숫자(NUMBER), 문자(VARSHAR2), 날짜(DATE) [자료형]
    v_sum NUMBER(10);
BEGIN
-- 2) 실행부 : BEGIN
-- 프로시저 코딩 : 업무 코딩(비즈니스 로직 코딩 : 실무 용어)
-- 사용법) 입력값 변수 : &변수명 사용
--        대입 연산자 - :=
--        v-sum    : 선언부에서 정의한 변수
...
-- 예제 2) 입력받은 사원 번호에 해당하는 월급 출력하기 : EMPLOYEE
-- 입력값 : 7788 (SCOTT)
-- 대화상자 출력
...
BEGIN
    ...
    -- 사용법) FOR 변수명(객체) IN 커서 변수명(배열) LOOP
    --        반복문(변수명.컬럼명);
    --        END LOOP;
...
-- 참고) 실무에서 사용하는 이름 있는 프로시저 형태
-- 사용법) CREATE OR REPLACE PROCEDURE 프로시저명
--                  (매개변수 in 자료형
--                   내보내기 변수 out 자료형 --js return 값 
--                    )
--       IS
--         (선언부)
--       BEGIN
...
-- 함수(Function) 특징 : 무조건 return 값이 있음(결과 내보내기 있음)
