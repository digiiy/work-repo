 ...
 * 인덱스 : 배열 항복에 붙힌 번호, 0 ~ n 까지의 범위
 * 배열 선언(정의) : 1) 자료형[] 변수 = {값1, ... 값n};
 * 2) 자료형[] 변수 = new 자료형[길이];
 * 3) 자료형[] 변수 = new 자료형[길이]{값1, ... 값n};
 * 배열 길이 : 배열의 값의 개수, 배열 변수.length (읽기 전용)
...
//     int sum3 = add({84, 91, 88}); // 에러 발생 이렇게 X
//     참고)
...
//        fori : 반복문 이용
        for (int i = 0; i < 3; i++) {
            newArray[i] = oldArray[i]; // 깊은 복사(원본값 바뀌워도 복사본이 바뀌지 않음)
