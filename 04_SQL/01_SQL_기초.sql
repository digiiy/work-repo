...
-- 예제1) 사원테이블의 모든 컬럼 정보를 보기하는 SQL문 작성하세요
-- 사원 테이블명 : EMPLOYEE
SELECT * FROM EMPLOYEE;

-- 예제2) 사원테이블의 컬럼중에서 ENO, ENAME 컬럼만 정보를 보는 SQL문을 작성하세요
SELECT ENO, ENAME FROM EMPLOYEE;

-- 예제3) 부서테이블의 컬럼중에서 DNO(부서번호), LOC(부서위치) 컬럼만 조회하기
-- 부서 테이블: DEPARTMENT
SELECT DNO, LOC FROM DEPARTMENT;

-- 예제4) 힌트 컬럼에 *(/,+,-;산술연산자) 연산해서 화면에 출력할 수 있습니다.
--       SALARY * 12 (1년 연봉)
-- 사원테이블 : EMPLOYEE
-- 월급 컬럼 : SALARY
-- 사원명 컬럼 : ENAME
-- 사원테이블에서 사원명과 1년연봉을 화면에 출력해보세요
SELECT ENAME, SALARY * 12 FROM EMPLOYEE;

-- 예제5) 사원테이블에서 커미션을 더한 연봉 구하기
-- 사원테이블 : EMPLOYEE
-- 월급 컬럼 : SALARY
-- 사원명 컬럼 : ENAME
-- 상여금 컬럼 : COMMISSION
...
-- NULL 특징 : NULL과 연산을 할 수 없음 (결과 : NULL)
-- 함수 : 사용법 : NVL(NUL값이_있는_컬럼, 변경될 값)
...
-- 예제 7) 중복된 데이터를 한번씩만 출력하게 하는 에제
-- 사용법) SELECT DISTINCT 중복 컬럼명 FROM 테이블명;
-- 사원테이블에 부서번호(DNO)를 중복을 제거해서 화면에 출럭하세요
SELECT DISTINCT DNO FROM EMPLOYEE;

-- 비교) 원본 DNO 질의문
SELECT DNO FROM EMPLOYEE;

-- 예제8) 간단한 산술연산 및 테스트 용도 테이블을 소개합니다.
-- DUAL 테이블 : 테스트 용도 테이블
SELECT * FROM DUAL;

-- 활용도)
SELECT 4*5+1 FROM DUAL;
-- SQL문의 문자열 : '문자열' : 홑따옴표 사용
