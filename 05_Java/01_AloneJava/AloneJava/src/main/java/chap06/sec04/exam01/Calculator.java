 ...
 *        함수 형태 :
 *            접근 제어자 리턴 자료형 함수명(매개변수1 ,...,매개변수n){ 실행문 ;}
 *            ex) public void sample(int a, String b, double c) {}
 *            접근 제어자 : public(공용), default(생략, package 내에서는 사용가능)
 *            함수 이름 명명법 : 낙타 표기법(카멜 표기법), 첫 글자는 소문자, 단어 + 단어는 대문자
 *            매개 변수 : 함수 실행시 전달되는 데이터
 *            리턴 자료형 : 함수가 실행된 이후 결과값을 내보내기 할때 사용
...
//    TODO: 함수 : 사물이나 사람의 동작(행동)을 나타냄
     ...
     *  사용법 : 접근제어자 리턴 자료형 함수명(매개변수1, ... 매개변수n){}
     *   접근 제어자 : public(공용*), private(개인*), default, protected
     */
    void powerOn(){
