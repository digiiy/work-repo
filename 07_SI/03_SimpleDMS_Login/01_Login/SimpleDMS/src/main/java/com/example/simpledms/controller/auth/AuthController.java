...
public class AuthController {
    ...
    public ResponseEntity<Object> login(@RequestBody UserReq userReq) {
        ...
        try {
            // 1) 인증관리자 시작 : id(email)/pwd 로 db에 있는지 조사
            // authentication : 인증을 통과한 객체(id/pwd,
            ...
            // =authentication.getPrincipa() : 순수한 유저정보(UserDetailsImpl : id/pwd)
    ...
    public ResponseEntity<Object> createUser(@RequestBody UserReq userReq) {
        try {
            ...
            // 3) 일단 리액트에서 요청한 권한이 있는지 조사
            String codeName = userReq.getCodeName(); // 요청 권한 가져오기

            // 임시 상태 체크 변수
            boolean bResult = false;

            // 4) codeName(리액트 요청 권한) 없으면 ROLE_USER 권한으로 신규 유저 생성
            if (codeName == null) {
...
//                에러 처리
            ...
            // 5) codeName(리액트 요청 권한) 있으면 그 요청 권한으로 신규 유저 생성
                switch (codeName) {
