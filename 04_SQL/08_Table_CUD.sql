...
-- 예) 최대 크기 : VARCHAR2(4000) 글자, VARCHAR2 : 잘 안씀(1 글자만 들어감)
...
-- 예제 2) DEPT 테이블 복사하기 : DEPT_COPY 테이블을 만드세요
-- 1) 데이터까지 포함해서 복사
...
COMMENT ON COLUMN EMP.SALARY     IS '입사일';
...
-- 컬럼 : DNO(NUMBER(2,0)), DAME(VARCHAR2(14), LOC(VARCHAR2(13))
...
-- 연습 2) DEPT_COPY 테이블에 50 부서 'COMPUTION' 만들고,
--        LOC 는 ' ' 공백으로 INSERT 하세요
INSERT INTO DEPT_COPY(DNO, DNAME, LOC)
VALUES(50, 'COMPUTION', ' ');         -- 데이터 생성
...
-- 에제 9) INSERT 할때 현재 날짜(시간) 데이터 넣기 : SYSDATE(현재날짜)
INSERT INTO EMP_COPY(ENO, ENAME, JOB, HIREDATE, DNO)
