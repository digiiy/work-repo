...
SELECT  LOWER('ENAME JOB')AS
       ,INITCAP('ENAME JOB')AS
FROM EMPLOYEE;
...
-- LENGTH(문자열) : 한글/영어 상관없이 문자갯수를 세어서 RETURN 하는 함수
...
-- 1-4) 문자 조작 함수 : 문지열 자르기
-- SQL : 인덱스번호(1부터 시작)
-- 사용법) SUBSTR(대상컬럼, 시작위치, 자를갯수)
-- 단, 시작위치가 음수이면 뒤에서 셈
SELECT SUBSTR('Oracle mania',4,3)
       ,SUBSTR('Oracle mania',-1,1)
FROM DUAL; 

-- 예제 3) 이름이 N으로 끝나는 사원명(ENAME) 화면에 출력하기
-- 단, SUBSTR() 함수를 이용하세요
-- 사원 : EMPLOYEE
...
-- 1-6) 컬럼에 어떤 문자를 붙이기 함수 : LAPD, RPAD
...
-- 1-7) 문자열에서 공백 제거하기 : LTRTM, RTRIM, TRIM
