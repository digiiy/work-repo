...
--    입사한 년도와(HIREDATE) 입사한 달(HIREDATE)만 출력하세요.
-- 사용법) SUBSTR(컬럼명, 인덱스 위치, 잘라낼 개수)
...
-- 2) 입사일(HIREDATE) 연도는 2자리(YY), 월은 (MON)으로 표시하고,
--    일(DD), 요일은 (DY)로 표기하세요. (날짜 포맷)
-- TO_CHAR(날짜 컬럼, '날짜 포맷') : 날짜 -> 문자로 바꾸는 함수 사용
-- 출력 예) 80/12월/17 수
-- 사원테이블 : EMPLOYEE
SELECT HIREDATE
...
-- 4) 사원들의(EMPLOYEE) 매니저(MANAGER) 사번(ENO)을 출력하되 매니저가 없는
