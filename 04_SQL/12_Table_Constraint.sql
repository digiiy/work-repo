...
-- 예제 1) CUSTOMER 테이블을 정의하고 ID컬럼에 UNIQUE 제약조건을 지정하세요
...
-- 2) 기본키 제약조건(중요) : PRIMARY KEY(PK)
--    기본키의 특징 : 유일한 값만 입력되고(중복방지) + NOT NULL 제약 조건 포함 + (참고 : INDEX 자동생성)
-- 특징) 주로 테이블 설계시 1개의 테이블당 거의 1개는 기본키가 존재함
-- 기본키 제약조건에 이름을 부여 : CONSTRANIT 제약조건 이름 제약조건
-- 제약조건 이름 붙이기 약속(명명법) : PK_테이블명_컬럼명, UK_테이블명_컬럼명 ...
-- 사용법) CREATE TABLE 테이블명(
--                컬럼명 자료형(크기)
...
-- 예제 1) 사원 복사본 테이블(EMP_COPY)에 기본키(PRIMARY KEY) 추가하기
...
-- 자식 테이블에 추가 (부서(부모): DEPT_COPY, 사원(자식) : EMP_COPY)
ALTER TABLE EMP_COPY
ADD CONSTRAINT PK_EMP_COPY_DNO
FOREIGN KEY(DNO) REFERENCES DEPT_COPY(DNO);

-- 예제 4) 테이블 생성후에 제약조건 삭제하기
-- 부모 테이블의 기본키 (부서:부서번호) <-> 자식 테이블의 참조키 (사원:부서번호)
-- 부모 테이블의 기본키 삭제하기 : 강제 삭제하기 옵션(CASCADE)
ALTER TABLE DEPT_COPY
