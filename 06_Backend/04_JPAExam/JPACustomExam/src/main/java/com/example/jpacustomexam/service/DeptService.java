...
//    전체 조회 + 정렬(dname 내림차순) : 쿼리 메소드
    ...
    //    전체 조회 + dname like(Containing) 검색 + 정렬(dname 내림차순)
...
//    1)
    public Page<Dept> findAllByDnameContaining(String danme, Pageable pageable) {
        Page<Dept> page
                = deptRepository.findAllByDnameContaining(danme, pageable);
