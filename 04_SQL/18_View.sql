...
-- 활욜) 1) 보안을 유지하기 위해 사용 : 사원테이블 - 급여컬럼(비밀 유지:대외비)
--      2) SQL 문을 미리 만들어 놓고 재활용 : 코딩 생산성 향상
-- 1) 뷰 생성
-- 사용법) CREATE OR REPLACE VIEW VW_EMP_JOB(사번, 사원이름, 부서번호, 담당업무)
--        AS
--        SELECT 원본컬럼명, 원본컬럼명2...
--        FROM 테이블명
--        WHERE 조건절
...
-- 예제 1) 부서 테이블(DEPARTMENT)과 사원 테이블을(EMPLOYEE)
...
-- 뷰 정보 확인 : 데이터 사전(시스템 테이블:관리용(미리 생성되어 있음)) : USER_VIEWS
SELECT * FROM USER_VIEWS;

-- 뷰에 INSERT 가능한가?
-- 예제 2) VW_EMP_JOB에 INSERT 하기
...
VALUES(9000, '김주현', 30, 'MANAGER'); -- 에러발생
